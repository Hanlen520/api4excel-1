...
class ApiTest:
    ...
    def run_test(self, sheet, url):
        ...
        fail = 0
        rb = xlrd.open_workbook("C:\\Users\\user\\Desktop\\pythonTest\\api4excel-master\\test.xls")
        ...
        for i in range(2, rows):
            ...
            actualCode = requests.request(testMethod, testUrl, data=json.dumps(testData), headers=testHeaders)
            ...
            failResults.add_row([testNumber, testMethod, testUrl, testData, actualCode.status_code, expectCode])
            response = json.loads(actualCode.text)
            ws.write(i,7,response["msg"])
            wb.save("C:\\Users\\user\\Desktop\\pythonTest\\api4excel-master\\2test.xls")
